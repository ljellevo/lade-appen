...
// Holds an object class for charger station for firebase realtime database
// 
// Paramter: key to use for chargerstaion
// Metods:
//      add(objToAdd,callback) - objToAdd must be in according to structure returned by chargerstation dump for NOBIL API
//      ShowData(bool)         - boolean to indicate pretty prinut out (colorized) or not.
// 
//
...
stationsObject.prototype.add = function(obj,callback) {
        ...
        for (i in obj.attr.conn[t]) {
            switch (i) {
                ...
                case "10":
                    ...
                    break;
                case "9":
                    myConn.OperationStatus=obj.attr.conn[t][i].trans;
                    break;
                case "8":
    ...
    self.data[obj.csmd.International_id]=myObj;
